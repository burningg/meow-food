<template>
  <view class="plan-page-root">
      ...
      <view class="page-shell plan-page" @tap="closePlanMenu">
        ...
        <section class="plan-list-section">
            ...
            <view
              ...
              :class="[
                'plan-card',
                {
                  ...
                  shopping: hasShoppingStarted(plan),
                },
              ]"
            >
              <view class="plan-card-head">
                <button
                  class="plan-card-main"
                  @tap="toggleExpandedPlan(plan.id)"
                >
                  <view class="plan-card-copy">
                    <text class="plan-card-title">{{ plan.title }}</text>
                    <text class="plan-card-meta"
                      >{{ plan.circleName }} ·
                      {{ plan.creatorNickname }}创建</text
                    >
                  </view>
                </button>
                <view class="plan-card-side">
                  ...
                  >
                    <text class="plan-card-badge-text">{{
                      getShoppingStatusLabel(plan.shoppingStatus)
                    }}</text>
                  </view>
                  <view v-if="plan.viewerCanManageRecipes" class="plan-card-menu-wrap">
                    <button
                      class="plan-card-menu-trigger"
                      @tap.stop="togglePlanMenu(plan.id)"
                    >
                      ···
                    </button>
                    <view
                      v-if="activeMenuPlanId === plan.id"
                      class="plan-card-menu-popover"
                      @tap.stop
                    >
                      <button
              ...
              <view v-if="expandedPlanId === plan.id" class="plan-card-body">
                ...
                <view v-else class="checked-empty-card">
                  ...
                  <text class="checked-empty-desc"
                    >先从圈内共享菜谱里挑几道，采购清单会自动跟着生成。</text
                  >
                </view>
          ...
          <view v-else class="empty-card plan-empty-card">
            <text class="plan-empty-title"
              >{{ selectedDateTitle }}还没有计划</text
            >
            <text class="plan-empty-desc"
              >{{ canUseOwnPlans ? '点右上角新建一条，让圈内成员一起补菜谱和采购清单。' : '这条分享计划还没有内容。' }}</text
            >
          </view>
        </section>
      </view>
    ...
    <view
      v-if="createSheetVisible"
      class="create-overlay"
      @tap="closeCreateSheet"
    >
      <section class="create-sheet" @tap.stop>
        ...
        <view class="create-sheet-fields">
          ...
          >
            <view class="create-field-card create-picker-card">
              ...
              <text
                :class="['create-picker-value', { empty: !selectedCircleName }]"
              >
                {{
                  selectedCircleName ||
                  (circles.length ? "选择圈子" : "先创建圈子")
                }}
              </text>
            </view>
          </picker>
        ...
        <view class="create-sheet-actions">
          <button class="create-cancel-button" @tap="closeCreateSheet">
            取消
          </button>
          <button
            v-if="circles.length"
          ...
          <button v-else class="create-submit-button" @tap="goCreateCircle">
            创建圈子
          </button>
        </view>
      </section>
...
const initialExpandedPlanId = routeParams.planId || "";
...
const selectedDate = computed(
  () => parseDateKey(selectedDateKey.value) || new Date(),
);
const selectedPlans = computed(
  () => plansByDate.value[selectedDateKey.value] || [],
);
const selectedDateTitle = computed(() =>
  formatDisplayDate(selectedDateKey.value, true),
);
const selectedDateShort = computed(() =>
  formatDisplayDate(selectedDateKey.value, false),
);
const circleOptions = computed(() =>
  circles.value.map((circle) => circle.name),
);
const selectedCircleIndex = computed(() => {
  const index = circles.value.findIndex(
    (circle) => circle.id === selectedCircleId.value,
  );
...
watch(
  ...
  async (plans) => {
    ...
    if (
      ...
      !plans.some((plan) => plan.id === activeMenuPlanId.value)
    ) {
      activeMenuPlanId.value = "";
    }
    if (!plans.length) {
    ...
    const hasExpandedPlan = plans.some((plan) => plan.id === expandedPlanId.value);
    if (!hasExpandedPlan) {
      // 仅在首次进入页面时默认展开第一条，后续允许用户全部收起。
      if (!hasAppliedInitialExpand.value) {
        expandedPlanId.value =
          plans.find((plan) => plan.id === initialExpandedPlanId)?.id ||
          plans[0].id;
...
async function loadInitial() {
  try {
    ...
    selectedCircleId.value =
      circlesData.find(
        (circle) => circle.id === profileData.lastSelectedCircleId,
      )?.id ||
      circlesData[0]?.id ||
      "";
    await loadWeek(true);
...
async function loadSharedPlan() {
  ...
  const restoredUser = await authStore.restore();
  if (restoredUser?.id) {
    currentUserId.value = restoredUser.id;
    try {
      const [{ data: circlesData }, { data: profileData }] = await Promise.all([
        socialService.getCircles(),
        socialService.getProfile(),
      ]);
      currentUserId.value = profileData.user.id || restoredUser.id;
      circles.value = circlesData;
      selectedCircleId.value =
        circlesData.find(
          (circle) => circle.id === profileData.lastSelectedCircleId,
...
async function loadWeek(force = false) {
  const monthKeys = Array.from(
    new Set(weekDates.value.map((item) => item.key.slice(0, 7))),
  );
...
function changeWeek(offset: number) {
  ...
  selectedDateKey.value = formatDateKey(
    addDays(viewedWeekStart.value, weekdayIndex),
  );
  void loadWeek();
}
...
async function removeRecipeFromPlan(
  planId: string,
  dishId: string,
  dishName: string,
) {
